...
public class APICustomEMCMapper implements IEMCMapper<NormalizedSimpleStack, Integer> {
    ...
    // Need a special Map for Items and Blocks because the ItemID-mapping might change, so we need to store
    // modid:unlocalizedName instead of the NormalizedSimpleStack which only holds itemid and metadata
    ...
    public void addMappings(IMappingCollector<NormalizedSimpleStack, Integer> mapper, Configuration config) {
        ...
        for (String modId : modIds) {
            ...
            if (customEMCforMod.containsKey(modId)) {
                for (Map.Entry<NormalizedSimpleStack, Integer> entry : customEMCforMod.get(modId)
                    .entrySet()) {
            ...
            if (customNonItemEMCforMod.containsKey(modId)) {
                for (Map.Entry<NormalizedSimpleStack, Integer> entry : customNonItemEMCforMod.get(modId)
                    .entrySet()) {
                    NormalizedSimpleStack normStack = entry.getKey();
                    if (isAllowedToSet(modId, normStack, entry.getValue(), config)) {
                        mapper.setValueBefore(normStack, entry.getValue());
