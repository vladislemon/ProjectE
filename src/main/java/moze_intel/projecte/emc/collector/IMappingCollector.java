...
public interface IMappingCollector<T, V extends Comparable<V>> {
    ...
    /**
     * Add a Conversion that produced {@code outnumber} items of {@code output} by consuming the ingredients, that are
     * specified in the {@code ingredientsWithAmount} Map.<br/>
     * The Map contains the ingredients and the amount of how many are consumed. It can contain negative amounts, if you
     * get multiple items back.
     * An amount of 0 will indicate, that the {@code output} should only get an EMC value assigned, if the
     * zero-amount-ingredient also has an EMC value.
     * 
     ...
     * {@code value} has to be >= 0 or {@link IValueArithmetic#isFree(Comparable) Free}, which indicates that
     * {@code something} can be used in
    ...
    /**
     * Set a fixed {@code value} for {@code something} after the calculation was done. This value will not be
     * propagated.<br/>
