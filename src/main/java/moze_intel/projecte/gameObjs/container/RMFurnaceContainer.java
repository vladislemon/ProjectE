...
public class RMFurnaceContainer extends Container {
    ...
    public RMFurnaceContainer(InventoryPlayer invPlayer, RMFurnaceTile tile) {
        ...
        // Input storage
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 4; j++) this.addSlotToContainer(new Slot(tile, i * 4 + j + 2, 11 + i * 18, 8 + j * 18));
        ...
        // Player HotBar
        for (int i = 0; i < 9; i++) this.addSlotToContainer(new Slot(invPlayer, i, 24 + i * 18, 142));
    }
    ...
    public void detectAndSendChanges() {
        ...
        for (int i = 0; i < this.crafters.size(); ++i) {
            ...
            if (lastCookTime != tile.furnaceCookTime) icrafting.sendProgressBarUpdate(this, 0, tile.furnaceCookTime);

            if (lastBurnTime != tile.furnaceBurnTime) icrafting.sendProgressBarUpdate(this, 1, tile.furnaceBurnTime);
    ...
    public void updateProgressBar(int par1, int par2) {
        if (par1 == 0) tile.furnaceCookTime = par2;

        if (par1 == 1) tile.furnaceBurnTime = par2;

        if (par1 == 2) tile.currentItemBurnTime = par2;
    }
