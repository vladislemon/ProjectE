...
public class MercurialEyeContainer extends Container {
    ...
    public MercurialEyeContainer(InventoryPlayer invPlayer, MercurialEyeInventory mercEyeInv) {
        ...
        // Hotbar
        for (int i = 0; i < 9; i++) this.addSlotToContainer(new Slot(invPlayer, i, 6 + i * 18, 114));
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotIndex) {
        ...
        } else // Moving from player inventory
        {
            if (((Slot) inventorySlots.get(0)).isItemValid(stack)
                && ((Slot) inventorySlots.get(0)).getStack() == null) { // Is a valid klein star and the slot is empty?
