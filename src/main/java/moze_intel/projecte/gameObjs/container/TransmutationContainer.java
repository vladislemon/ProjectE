...
public class TransmutationContainer extends Container {
    ...
    public TransmutationContainer(InventoryPlayer invPlayer, TransmutationInventory inventory) {
        ...
        // Player Hotbar
        for (int i = 0; i < 9; i++) this.addSlotToContainer(new Slot(invPlayer, i, 35 + i * 18, 175));
    ...
    private boolean isNeiScrollWheel() {
        final int stacktraceDepth = 3; // [getStackClasses(), isNeiScrollWheel(), slotClick(), [POSSIBLE POSITION FOR
                                       // NEI IN STACKTRACE]
