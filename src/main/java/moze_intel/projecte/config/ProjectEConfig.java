...
public final class ProjectEConfig {
    ...
    public static void init(File configFile) {
        ...
        try {
            ...
            showUnlocalizedNames = config.getBoolean(
                "unToolTips",
                "misc",
                false,
                "Show item unlocalized names in tooltips (useful for custom EMC registration)");
            showODNames = config.getBoolean(
                "odToolTips",
                "misc",
                false,
                "Show item Ore Dictionary names in tooltips (useful for custom EMC registration)");
            showEMCTooltip = config
                .getBoolean("emcToolTips", "misc", true, "Show the EMC value as a tooltip on items and blocks");
            showStatTooltip = config
                .getBoolean("statToolTips", "misc", true, "Show stats as tooltips for various ProjectE blocks");
            showPedestalTooltip = config
                .getBoolean("pedestalToolTips", "misc", true, "Show DM pedestal functions in item tooltips");
            showPedestalTooltipInGUI = config.getBoolean(
                "pedestalToolTipsInGUI",
                "misc",
                false,
                "Show pedestal function tooltips only in pedestal GUI");
            useLootBalls = config.getBoolean(
                "useLootBalls",
                "misc",
                true,
                "Make loot balls for drops. Disabling this may potentially cause bad performance when large amounts of loot are spawned!");
            pulsatingOverlay = config
                .getBoolean("pulsatingOverlay", "misc", false, "The Philosopher's Stone overlay softly pulsates");
            unsafeKeyBinds = config.getBoolean(
                "unsafeKeyBinds",
                "misc",
                false,
                "False requires your hand be empty for Gem Armor Offensive Abilities to be readied or triggered");
            projectileCooldown = config.getInt(
                "projectileCooldown",
                "misc",
                0,
                0,
            ...
            gemChestCooldown = config.getInt(
                "gemChestCooldown",
                "misc",
                0,
                0,
            ...
            craftableTome = config
                .getBoolean("craftableTome", "difficulty", false, "The Tome of Knowledge can be crafted.");
            altCraftingMat = config.getBoolean(
                "altCraftingMat",
                "difficulty",
                false,
                "If true some ProjectE items require a nether star instead of a diamond.");
            useOldDamage = config.getBoolean(
                "useOldDamage",
                "difficulty",
                false,
                "If true the old damage amounts from ProjectE 1.4.7 and before will be used for weapons.");
            offensiveAbilities = config.getBoolean(
                "offensiveAbilities",
                "difficulty",
                true,
                "Set to false to disable Gem Armor offensive abilities (helmet zap and chestplate explosion)");
            katarDeathAura = config.getFloat(
                "katarDeathAura",
                "difficulty",
                1000F,
                0,
            ...
            config.getCategory("pedestalcooldown")
                .setComment(
            ...
            archangelPedCooldown = config.getInt(
                "archangelPedCooldown",
                "pedestalcooldown",
                40,
                -1,
                Integer.MAX_VALUE,
            ...
            evertidePedCooldown = config.getInt(
                "evertidePedCooldown",
                "pedestalcooldown",
                20,
                -1,
            ...
            harvestPedCooldown = config.getInt(
                "harvestPedCooldown",
                "pedestalcooldown",
                10,
                -1,
            ...
            ignitePedCooldown = config.getInt(
                "ignitePedCooldown",
                "pedestalcooldown",
                40,
                -1,
            ...
            repairPedCooldown = config.getInt(
                "repairPedCooldown",
                "pedestalcooldown",
                20,
                -1,
            ...
            volcanitePedCooldown = config.getInt(
                "volcanitePedCooldown",
                "pedestalcooldown",
                20,
                -1,
                Integer.MAX_VALUE,
            ...
            timePedMobSlowness = config.getFloat(
                "timePedMobSlowness",
                "effects",
                0.10F,
                0.0F,
